...
This is the sequence to run in your live demo:

1. `docker compose up -d` — show the broker starting.
2. Start `consumer.py` in one terminal — point out it's subscribed and idle.
3. Start `producer.py --rate 2` in another terminal — show orders flowing
   through and the running average updating in the consumer.
4. Stop the producer (Ctrl+C), then demonstrate the DLQ paths on demand:
   ```bash
   python producer.py --bad-price   # invalid business rule -> instant DLQ, no retries
   python producer.py --corrupt     # not valid Avro -> instant DLQ, no retries
   ```
   Point out in the consumer's logs that these are *not* retried — only the
   simulated transient failures are.
5. Optionally bump `--failure-rate` up when starting the consumer
   (e.g. `python consumer.py --failure-rate 0.6`) to reliably trigger a
   `MAX_RETRIES_EXCEEDED` DLQ case live, if the organic 20% rate hasn't shown one yet.
6. View what's in the DLQ:
   ```bash
   python dlq_consumer.py
   ```
   This prints every DLQ record with its `errorReason`, `retryCount`, and
   `failedAtIso`, proving all three failure paths landed there correctly.
...
git init
git add .
git commit -m "Kafka order pipeline: Avro, retry logic, DLQ"
git remote add origin <your-repo-url>
git push -u origin main
```

`.gitignore` already excludes `venv/` and `__pycache__/` so your repo stays clean.

## Project structure

```
.
├── docker-compose.yml       # single-node Kafka (KRaft mode)
├── schemas/
│   ├── order.avsc           # main order message schema
│   └── order_dlq.avsc       # DLQ record schema (order fields + failure metadata)
├── producer.py               # generates + sends order messages
├── consumer.py               # consumes, aggregates, retries, routes to DLQ
├── dlq_consumer.py           # read-only viewer for the DLQ topic
├── requirements.txt
└── README.md
```
